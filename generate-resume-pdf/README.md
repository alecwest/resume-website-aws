...
```bash
generate-resume-pdf$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
generate-resume-pdf$ sam local start-api
generate-resume-pdf$ curl http://localhost:3000/
```
...
```bash
generate-resume-pdf$ sam logs -n HelloWorldFunction --stack-name generate-resume-pdf --tail
```
...
```bash
generate-resume-pdf$ cd hello-world
...
```bash
aws cloudformation delete-stack --stack-name generate-resume-pdf
```
